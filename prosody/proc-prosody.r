...
get.var.aggs.window <- function(x,xprops, var.name="F0") {
	...
	#print(c(currconv, currpart, currspk))
	...
	## Get aggs on slope corrected features 
...
calc.spk.aggs <- function(x, var.name="F0", xconv="Xconv", xparticipant="Xpart", xspk="Xspk") {
	...
	#print(names(x))
        if (nrow(x) > 0) {
			...
			lc <- get.legendre.coeffs(data.table(Time=Time, val=val), degree=5, xname="Time", yname="val")
			...
			lc1.val=lc[1], lc2.val=lc[2], lc3.val=lc[3], lc4.val=lc[4], lc5.val=lc[5],
			...
			lc1.val=NA, lc2.val=NA, lc3.val=NA, lc4.val=NA, lc5.val=NA,
